...
curl -s -L \
  -H "Authorization: Bearer $REPLICATE_API_TOKEN" \
  -o "./references/{filename}" "{output_url}"
```

> **중요**: Replicate 출력 URL은 Authorization 헤더가 필요합니다. `-L` 플래그로 리다이렉트도 따라갑니다.

**파일명 규칙**: `{timestamp}_{slug}.{ext}`
- `timestamp`: `YYYYMMDD_HHmmss` 형식 (예: `20260327_143052`)
- `slug`: 프롬프트에서 추출한 짧은 영문 슬러그 (공백→하이픈, 소문자, 최대 30자)
- `ext`: 파일 확장자 (이미지: `webp` 또는 `png`, 동영상: `mp4`)

예시: `20260327_143052_cyberpunk-city-neon.webp`

복수 생성 시 접미사 추가: `20260327_143052_cyberpunk-city-neon_01.webp`
...
| 항목 | 값 |
|------|-----|
| 모델 | black-forest-labs/flux-2-pro |
| 프롬프트 | Cyberpunk city nightscape with neon lights |
| 비율 | 1:1 |
| 생성 수 | 1장 |
| 소요 시간 | 12.3초 |
| 저장 경로 | ./references/20260327_143052_cyberpunk-city-neon.webp |

생성된 파일:
- `./references/20260327_143052_cyberpunk-city-neon.webp`
```
