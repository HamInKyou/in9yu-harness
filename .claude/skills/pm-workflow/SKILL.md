...
사용자에게 안내합니다:
```
"아이디어를 구체화하겠습니다. 다음 중 선택하세요:"
1. `/discover` — 풀 디스커버리 사이클 (아이디어→가정 검증→실험 설계)
2. `/brainstorm` — 빠른 브레인스토밍 (PM/Designer/Engineer 관점)
3. "이미 구체적인 기능이에요" — 바로 PRD로
```

**Step 2: PRD 작성**
사용자에게 안내합니다:
```
"기능이 구체화되었습니다. PRD를 작성할까요?"
→ `/write-prd` 안내
```
...
**Step 4: GitHub 이슈 생성**
유저 스토리를 GitHub 이슈로 변환합니다:
```bash
# 각 스토리별 이슈 생성
gh issue create --title "스토리 제목" --body "스토리 내용" --label "enhancement"
```
- 사용자에게 생성할 이슈 목록을 보여주고 확인을 받습니다
- 확인 후 `gh issue create`로 일괄 생성합니다
...
작업할 이슈를 선택하고 워크플로우를 시작합니다:

**Step 1: 작업할 이슈 선택**
컨텍스트의 열린 이슈 목록을 표시하고 선택을 요청합니다:
```
"어떤 이슈를 작업할까요?"
[열린 이슈 목록 표시]
```
...
**Step 2: 워크플로우 시작**
```
"이슈 #N을 작업합니다. 실행 방식을 선택하세요:"
1. `/gh-workflow N` — 브랜치만 생성 (직접 코딩)
2. `/gh-workflow N --full` — 자동 구현 + PR
3. `/autopilot` — OMC 자율 실행
4. `/ralph` — 반복 루프 (검증까지 자동)
```

**Step 3: 완료 후**
```
"작업이 완료되면:"
- `/gh-workflow N --pr` — PR 생성
- `/gh-project move 1 #N Done` — 보드 상태 변경
- `/simplify` — 코드 품질 검토
```
...
- **스킬 안내**: 직접 실행하지 않고 적절한 스킬을 안내합니다 (사용자가 스킬을 배우도록)
